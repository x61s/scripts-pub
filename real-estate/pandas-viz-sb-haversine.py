...
print('Nearest coastline points for each property object:')
...
# nearest highway points for each df property object
print('Nearest highway points for each property object:')

hwArray = []
for key, value in highway.items():
    hwArray.append((value['Lon'], value['Lat']))

for index, row in df.iterrows():
    prop = [(float(row['Lon']), float(row['Lat']))]
    index, hwDist = nearest(prop, hwArray)
    print(hwArray[index], hwDist)
...
ax.set_title('Distance To The Sea [Haversine Formula]')
